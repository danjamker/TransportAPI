...
function TransportAPIError (errors) {
	...
	TransportAPI.prototype.BusDepartures = function BusDepartures (atcocode, date, time, format ,options, callback) {
		  if (typeof options === 'function') {
		    callback = options;
		    options = {};
		  }
		  var url = this.url + "/bus/stop/" + atcocode +"/"+date+"/"+time+"/timetable." + format;
		  options["api_key"] = this.APIKey
		  options["app_id"] = this.APIID
		  var query = _.reduce(options, function (result, val, key) {
		    if (result) return result + '&' + key + '=' + val;
